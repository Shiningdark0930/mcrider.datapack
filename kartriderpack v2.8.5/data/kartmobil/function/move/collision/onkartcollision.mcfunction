...
execute on passengers at @s[tag=kartdirection,type=marker] on vehicle positioned ~ ~0.2 ~ positioned ^ ^ ^-1.5 run tag @a[gamemode=spectator,distance=..2] add kart-listener-collision
execute as @n[tag=carB,type=item_display] on passengers at @s[tag=kartdirection,type=marker] on vehicle positioned ~ ~0.2 ~ positioned ^ ^ ^-1.5 run tag @a[gamemode=spectator,distance=..2] add kart-listener-collision

#방향 엔티티
tag @n[tag=kartdirection,type=marker] add carAdirection
execute at @n[tag=carB,type=item_display] run tag @n[tag=kartdirection,tag=!carAdirection,type=marker] add carBdirection
