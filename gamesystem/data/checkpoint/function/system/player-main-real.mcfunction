...
#만약 내가 통과(체크포인트 주변에 접근)한 체크포인트가 B의 번호와 같다면, A와 B의 숫자를 각각 1씩 증가시키는 체크포인트 통과함수를 1회 실행함.
execute positioned ~-15 -64 ~-15 as @e[dx=30,dy=384,dz=30,tag=check-point,tag=!check-large,type=marker] if score @s check-num = @a[tag=check-player-temp,limit=1] check-next as @p[tag=check-player-temp] at @s run function checkpoint:system/check-pass
execute positioned ~-30 -64 ~-30 as @e[dx=60,dy=384,dz=60,tag=check-point,tag=check-large,type=marker] if score @s check-num = @a[tag=check-player-temp,limit=1] check-next as @p[tag=check-player-temp] at @s run function checkpoint:system/check-pass
