...
#11초(220) 안에 완주
bossbar set minecraft:license max 61750

bossbar set minecraft:license name {"text":"01:01.750 내로 완주","color":"aqua"}

execute store result bossbar minecraft:license value run scoreboard players get time timermain

execute if score time timermain matches 61751.. run function license:system/retirefail
#완주 못하면 리타이어
...
#결승선을 지나기
execute if block ~ ~-1.6 ~ magma_block if entity @s[tag=check-pass-last] run function license:system/pro/mission3complete
