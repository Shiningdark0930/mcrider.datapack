...
#일반 부스터 파티클
execute if score @s kartboosttime matches 26.. unless score @s kartboosttime matches 32..40 on passengers rotated as @s[tag=kartdirectiontemp,type=item_display] rotated ~ 0 run particle minecraft:enchanted_hit ^ ^1 ^1 .25 .25 .25 1.25 1

#듀얼부스터 이펙트
execute if score @s kartboosttime matches 37 run function kartmobil:move/boost-effect/1.0/dualboost-transform-particle
execute if score @s kartboosttime matches 25 run function kartmobil:move/boost-effect/1.0/dualboost-burst-particle
execute if score @s kartboosttime matches ..25 run function kartmobil:move/boost-effect/1.0/dualboost-particle
...
execute if score @s kartboosttime matches 0 run scoreboard players set @p[tag=kartpassenger,distance=..0.3] kartboosttime 4