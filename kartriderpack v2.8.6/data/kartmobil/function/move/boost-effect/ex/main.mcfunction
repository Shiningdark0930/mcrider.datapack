...
#일반 부스터 파티클
execute if score @s kartboosttime < #ex-dual-boost-end-time kartboostduration unless score @s kartboosttime matches 32..40 on passengers rotated as @s[tag=kartdirectiontemp,type=item_display] rotated ~ 0 run particle minecraft:enchanted_hit ^ ^1 ^1 .25 .25 .25 1.25 1 normal @a
...
execute if score @s kartboosttime = #ex-dual-boost-end-time kartboostduration on passengers rotated as @s[tag=kartdirectiontemp,type=item_display] rotated ~ 0 run particle minecraft:cloud ^ ^1 ^2 .25 .25 .25 0.75 12 force @a[tag=kart-listener]
