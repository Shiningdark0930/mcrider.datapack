...
#페달조작
execute if entity @p[tag=kartpassenger,predicate=kartmobil:w,distance=..0.3] run function kartmobil:control/gear-mode/accel
execute if entity @p[tag=kartpassenger,predicate=kartmobil:s,distance=..0.3] run function kartmobil:control/brake

#기어조작
execute unless score @s[tag=kart-boost-press] kartboostcount matches 6.. rotated as @p[tag=kartpassenger,predicate=kartmobil:d,distance=..0.3] run scoreboard players add @s kartboostcount 1
